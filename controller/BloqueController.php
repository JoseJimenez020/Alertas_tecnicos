...
class BloqueController
{
    ...
    public function update(): void
    {
        ...
        if (!$model->findById($id))
            $this->jsonError('Bloqueo no encontrado.', 404);
    ...
    {
        if (
            empty($body['tecnico_id']) || empty($body['motivo'])
            || empty($body['fecha_inicio']) || empty($body['fecha_fin'])
        ) {
            $this->jsonError('Faltan campos obligatorios.', 422);
        }
        if (!in_array($body['motivo'], ['mecanico', 'vacaciones', 'apoyo', 'no_se_presento'])) {
