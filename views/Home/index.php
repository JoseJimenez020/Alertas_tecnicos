...
<head>
    ...
    <style>
        ...
        #liveIndicator.error .live-dot {
            background: #c0392b;
            animation: none;
        }

        #liveIndicator.syncing .live-dot {
            background: #ffc107;
            animation: none;
        }
        ...
        .cell-updated {
            animation: cellFlash .9s ease-out forwards;
        }
        ...
        td.cell-ticket:hover {
            background: #f0f7ff;
        }

        td.cell-ticket.occupied {
            cursor: pointer;
        }

        td.cell-ticket.estado-terminado {
            background-color: #d4edda !important;
        }

        td.cell-ticket.estado-terminado:hover {
            background-color: #c3e6cb !important;
        }

        td.cell-ticket.estado-primera-llamada {
            background-color: #637052 !important;
        }

        td.cell-ticket.estado-segunda-llamada {
            background-color: #E48312 !important;
        }

        td.cell-ticket.estado-rojo {
            background-color: #fa2e2e !important;
        }

        td.cell-ticket.estado-rojo:hover {
        ...
        .llamada-bloque .llamada-fields {
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 8px;
        }

        .llamada-bloque label {
            margin-top: 4px;
        }
        ...
        .btn-tecnico-status:hover {
            color: #e67e00;
        }

        /* ── Modal de reagendado ─────────────────────────────────── */
        #modalReagendar .modal-box {
            width: 460px;
        }
        ...
        #rSelectTecnico optgroup,
        #rSelectSlot optgroup {
            font-weight: bold;
            color: #1a4d6d;
        }
...
<body>
    ...
    $allTecs = $tecnicosAll ?? [];
    if (empty($allTecs)) {
        foreach ($tecnicosGroup as $lista)
            foreach ($lista as $t)
                $allTecs[] = $t;
    }
    ...
    foreach ($allTecs as $t) {
        if (!in_array($t['zona_nombre'] ?? '', $zonaOrder))
            $tecnicosSorted[] = $t;
    }
    $seen = [];
    $tmp = [];
    foreach ($tecnicosSorted as $t) {
        if (!isset($seen[$t['TecnicoId']])) {
            $seen[$t['TecnicoId']] = true;
            $tmp[] = $t;
        }
    }
    $tecnicosSorted = $tmp;
                ...
                <?php foreach ($horarios as $h): ?>
                    <tr>
                        ...
                        <?php foreach ($tecnicosSorted as $t):
                            ...
                            ?>
                            <td class="<?= $cellClass ?>" data-tecnico-id="<?= $t['TecnicoId'] ?>"
                                ...
                                data-fecha="<?= htmlspecialchars($fecha) ?>" <?= $hasTicket ? 'data-ticket-id="' . $ticket['ticket_id'] . '"' : '' ?>
                    ...
                    <?php foreach ($zonas as $z):
                        ...
                        <?php foreach ($allTecsByZona[$z] as $t):
                            ...
                            <div class="<?= $liClass ?>">
                                ...
                                <span class="list-item-nombre"><?= htmlspecialchars(strtoupper($t['TecnicoNombre'])) ?>
                                    <b><?= htmlspecialchars(strtoupper($t['num_telefono'])) ?></b> </span>
                                <?php if (!$disp && $mot): ?>
                                    <span class="motivo-tag">(<?= htmlspecialchars($mot) ?>)</span>
                                <?php endif; ?>
                                <button class="btn-tecnico-status" data-tecnico-id="<?= $t['TecnicoId'] ?>"
                                    data-tecnico-nombre="<?= htmlspecialchars($t['TecnicoNombre']) ?>" data-bloqueo-id="<?= $bId ?>"
                                    data-motivo="<?= htmlspecialchars($mot) ?>" data-inicio="<?= $bInicio ?>"
                                    data-fin="<?= $bFin ?>" data-desc="<?= $bDesc ?>" data-horas="<?= $bHoras ?>"
                                    onclick="openEditTecnicoModal(this)" title="Cambiar disponibilidad">✏️</button>
                            </div>
                        <?php endforeach; ?>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="modal-box">
            ...
            <div class="modal-body">
                ...
                <div id="wrapCajaPuerto" style="display:none;">
                    ...
                    <button id="btnGuardarCajaPuerto" onclick="guardarCajaPuerto()" style="display:none; margin-top:6px; padding:5px 14px; font-size:11px;
                ...
                <div class="llamadas-section" id="llamadasSection" style="display:none;">
                    ...
                    <?php for ($n = 1; $n <= 3; $n++): ?>
                        <fieldset class="llamada-bloque" id="llamadaBloque<?= $n ?>">
                            ...
                            <button class="btn-save-llamada" onclick="saveLlamada(<?= $n ?>)">💾 Guardar Llamada
                                <?= $n ?></button>
                    ...
                    <fieldset class="llamada-bloque" id="llamadaBloque4" style="border-color:#1a7a4a;">
                        ...
                        <button class="btn-save-llamada" style="background:#1a7a4a;" onclick="saveLlamada(4)">Guardar
                            Llamada de Calidad</button>
            ...
            <div class="modal-body">
                ...
                <div class="reagendar-step active" id="rStep1">
                    <div class="dias-selector">
                        ...
                        <button class="btn btn-primary" style="padding:5px 12px;" onclick="cargarSlots()">Buscar
                            slots</button>
                    </div>
                ...
                <div class="reagendar-step" id="rStep2">
                    ...
                    <div id="rConfirmInfo"
                        style="background:#f0f7ff;border:1px solid #1a4d6d;padding:10px;font-size:12px;border-radius:3px;">
                    </div>
                </div>
            </div>
    ...
    `;
        ...
        function resetModal() {
            ...
            document.getElementById('rescheduleResult').style.display = 'none';
            document.getElementById('fCajaPuerto').value = '';
            document.getElementById('btnGuardarCajaPuerto').style.display = 'none'; // <- nuevo
        ...
        async function guardarCajaPuerto() {
            ...
            const json = await res.json();
            showFeedback(
                json.success ? '✓ Caja/Puerto guardado.' : (json.message || 'Error al guardar.'),
